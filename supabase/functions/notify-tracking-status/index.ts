...
//
// ⚠ LLAVE GENERAL (2026-09-25, D006 / sql/068, pedido de Luis): respeta
// app_settings.wa_envio_automatico igual que bot_flush_outbox:
//   '0' (o sin fila)  → no manda nada y NO toca la cola (queda para cuando se prenda)
//   'prueba'          → sólo a números de wa_envio_contactos (hoy: Thomy); el resto se marca
//                       error_msg='held_no_whitelist' y no se reintenta
//   '1'               → producción
...
async function modoEnvio(): Promise<string> {
  const { data, error } = await supabase
    .from("app_settings").select("value").eq("key", "wa_envio_automatico").maybeSingle();
  if (error || !data) return "0"; // fail-closed
  const v = String(data.value ?? "").trim();
  return v === "prueba" || v === "1" ? v : "0";
}
async function numerosDePrueba(): Promise<Set<string>> {
  const { data } = await supabase.from("wa_envio_contactos").select("phone");
  return new Set((data ?? []).map((r: { phone: string }) => digits(r.phone)).filter(Boolean));
}
...
Deno.serve(async (req) => {
  ...
  // 0b. Llave general (D006). Apagada → no se lee ni se toca la cola.
  const modo = await modoEnvio();
  if (modo === "0") {
    return json({ ok: true, processed: 0, skipped: "llave_apagada" });
  }
  const prueba = modo === "prueba" ? await numerosDePrueba() : null;
  ...
  for (const n of pendings) {
    ...
    // Modo prueba: sólo números de wa_envio_contactos.
    if (prueba && !prueba.has(digits(cust.whatsapp))) {
      await supabase.from("bot_pending_notifications").update({
        error_at: new Date().toISOString(),
        error_msg: "held_no_whitelist",
      }).eq("id", n.id);
      held++;
      continue;
    }
  ...
  return json({ ok: true, modo, processed: pendings.length, sent, skipped, held, failed });
});
