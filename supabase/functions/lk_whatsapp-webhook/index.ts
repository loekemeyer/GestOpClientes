...
function extractCuit(text: string): string | null {
  // Formato XX-XXXXXXXX-X o solo dígitos
...
async function handleRegistration(
  ...
  if (!cuit) {
    // No parece un CUIT — pedir identificación
    const reply =
  ...
  switch (result.status) {
    ...
    case "pending_primary": {
      // El cliente ya tiene un teléfono principal — requiere aprobación
      await sendText(
        cfg.waPhoneId, cfg.waToken, phone,
      ...
      // TODO: notificar al teléfono principal para que apruebe
      break;
    }
...
async function handleMessage(
  ...
  if (modo === "humano") {
    // Modo humano — guardar mensaje pero no responder (el agente lo ve en inbox)
  ...
  if (!customer) {
    // No identificado — flujo de registro
